...
const User = ({ userdata, setLevelFilter, setRoleFilter, langFilter, setlangFilter, toolFilter, setToolFilter }) => {
    ...
    return (
        <div className={`bg-white shadow-md flex flex-col md:flex-row md:items-center md: justify-between md:px-8 md:py-2 md:gap-1 rounded mx-5 md:mx-20 mt-6 mb-8 p-3 ${featured && 'border-l-4 border-teal-600'} `}>
            ...
            <div className='mt-4 flex flex-wrap gap-x-6 gap-y-4' >
                {role && <span className="bg-teal-50 text-teal-600 py-1 px-2 font-semibold rounded-md cursor-pointer hover:bg-teal-600 hover:text-white" onClick={() => setRoleFilter(role)} > {role} </span>}
                {level && <span className="bg-teal-50 text-teal-600 py-1 px-2 font-semibold rounded-md cursor-pointer hover:bg-teal-600 hover:text-white" onClick={() => setLevelFilter(level)}> {level} </span>}

                {
                    languages !== '' ?
                        languages.map((language, index) => (
                            <span className="bg-teal-50 text-teal-600 py-1 px-2 font-semibold rounded-md cursor-pointer hover:bg-teal-600 hover:text-white" key={index}
                                onClick={() => {
